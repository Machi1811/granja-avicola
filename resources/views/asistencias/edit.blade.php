...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-2">✏️ Editar Asistencia</h1>
        <p class="text-gray-600">{{ $asistencia->fecha->format('d/m/Y') }} - {{ $asistencia->operario->nombre_completo }}</p>
    </div>
    ...
    <!-- Formulario -->
    <form action="{{ route('asistencias.update', $asistencia) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-4">
            ...
            <div>
                ...
                <select name="operario_id" class="w-full p-4 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:outline-none" required>
                    @foreach($operarios as $operario)
                        <option value="{{ $operario->id }}" {{ old('operario_id', $asistencia->operario_id) == $operario->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <div class="flex gap-4">
                    <label class="flex-1 cursor-pointer">
                        <input type="radio" name="asistio" value="1" class="hidden peer" {{ old('asistio', $asistencia->asistio) == 1 ? 'checked' : '' }} required>
                    ...
                    <label class="flex-1 cursor-pointer">
                        <input type="radio" name="asistio" value="0" class="hidden peer" {{ old('asistio', $asistencia->asistio) == 0 ? 'checked' : '' }}>
            ...
            <div>
                ...
                <textarea name="observaciones" 
                          rows="3" 
                          class="w-full p-4 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:outline-none"
                          placeholder="Motivo de inasistencia u observaciones...">{{ old('observaciones', $asistencia->observaciones) }}</textarea>
            </div>
        </div>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('asistencias.show', $asistencia) }}" class="bg-gray-300 text-gray-700 py-4 px-6 rounded-lg text-xl font-bold hover:bg-gray-400 transition">
                ← Cancelar
            </a>
        </div>
    </form>
